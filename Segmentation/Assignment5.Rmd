...
cust_data.df_test2 <- subset(customer_data, select = c("Region","DebtToIncomeRatio"))
...
customer_data$HHIncome = as.integer(gsub("[\\$,]", "", customer_data$HHIncome))
mean(customer_data$HHIncome, na.rm = TRUE)

# summary(customer_data$HHIncome)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 9000   24000   38000   54760   67000 1073000 
customer_data$CardSpendMonth = as.integer(gsub("[\\$,]", "", customer_data$CardSpendMonth))
# 
# mean(customer_data$CardSpendMonth, na.rm = TRUE)
...
# summary(customer_data$CardSpendMonth)
  # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
...
Gender Recoded: 1: female 2: male
```{r}
...
# This is a qualitative judgment effort and depends on the theme that you have selected for your 
# segmentation
...
customer_data%>%select_if(is.numeric)%>%cor()%>%ggcorrplot::ggcorrplot()
```
I will first segment customers based on demographics. This means their age, income, education, Retired, gender, region, 

CarOwnership Recoded: 1: Own, 2: Lease, 0: Does not own
...
# Recoding politicalpartymem,maritalstatus to binary
customer_data$PoliticalPartyMem<-ifelse(customer_data$PoliticalPartyMem == "Yes", 1, 0)

customer_data$Votes<-ifelse(customer_data$Votes== "Yes", 1, 0)

customer_data$UnionMember<-ifelse(customer_data$UnionMember=="Yes",1,0)


customer_data$MaritalStatus<-ifelse(customer_data$MaritalStatus=="Married",1,0)

customer_data$Retired<-ifelse(customer_data$Retired=="Yes",1,0)

customer_data$CarOwnership<-ifelse(customer_data$CarOwnership=="Own",1,
                                   ifelse(customer_data$CarOwnership=="Lease",2,0)
                                   )


customer_data <- mutate(customer_data, EmploymentLength = case_when(
 EmploymentLength%in%  0:10      ~ 1,
 EmploymentLength %in%  11:20     ~ 2,
 EmploymentLength %in%  21:30     ~ 3,
 EmploymentLength %in%  31:40     ~ 4,
 EmploymentLength %in%  41:52     ~ 5
 )
)  
                                   

customer_data <- mutate(customer_data, JobCategory = case_when(
 JobCategory=="Professional" ~ 1,
 JobCategory== "Sales"      ~ 2,
 JobCategory=="Service"     ~ 3,
 JobCategory=="Labor"     ~ 4,
 JobCategory=="Agriculture"    ~ 5,
 JobCategory=="Crafts"~6,
 JobCategory==""~0
 )
)

cust_data.df.num <- subset(customer_data,select=c("Region","TownSize","Age",
"JobCategory","UnionMember","EmploymentLength","Retired","CreditDebt",
"OtherDebt","MaritalStatus","HouseholdSize","HomeOwner","CarsOwned",
"CarOwnership","PoliticalPartyMem","Votes","educ_years_rec" ,"HHIncome_rec", "LoanDefault_num","Gender_num","DebtToIncomeRatio_rec"
))
...
lowerCor(select_if(customer_data,is.numeric))
...
complete.cases(cust_data.df.num)


cust_data.df.num<-cust_data.df.num%>%mutate(TownSize=replace_na(TownSize,2))
...
```{r}
plot(cust_data.df.num$HHIncome ~cust_data.kmeans_seg_5$cluster, ylab = "Income", xlab = "Cluster")
...
sol_5 <-fviz_cluster(cust_data.kmeans_seg_5, cust_data.df.num)
sol_5
```
